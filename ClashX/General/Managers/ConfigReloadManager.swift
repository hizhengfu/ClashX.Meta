...
final class ConfigReloadManager {
    ...
    private func syncConfigKeepingTunEnabledIfNeeded(_ shouldKeepTunEnabled: Bool) async {
    ...
    func updateConfig(configName: String? = nil, showNotification: Bool = true) async -> ErrorString? {
        let shouldKeepTunEnabled = ConfigManager.shared.proxyState.isTunModeActive
        ...
        await syncConfigKeepingTunEnabledIfNeeded(shouldKeepTunEnabled)
        resetStreamApi()
